...
{
    public class SuperPickerBoxBooster : BoosterBase
    {
        ...
        public override bool IsValidTarget(BoxController target)
        {
            ...
            if (target.IsCleared || target.IsFull) return false;
            if (grid != null && !grid.IsStandardBox(target.GridPosition)) return false;
            return true;
        }
        ...
        public override IEnumerator Execute(BoxController target = null)
        {
            ...
            yield return level.RunSuperPickerFillCoroutine(target);
