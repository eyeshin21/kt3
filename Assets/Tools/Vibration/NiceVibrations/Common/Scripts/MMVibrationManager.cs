...
namespace MoreMountains.NiceVibrations
{
    ...
    public static class MMVibrationManager
    {
        ...
        public static void AndroidVibrate(long milliseconds, int amplitude)
        {
            ...
            {
                string manufacturer = new AndroidJavaClass("android.os.Build").GetStatic<string>("MANUFACTURER").ToLower();
                long tempDuration = milliseconds;
                int tempAmplitude = amplitude;

                // Custom cho từng hãng
                switch (manufacturer)
                {
                    case "samsung":
                        tempAmplitude = amplitude + 60;
                        tempDuration = milliseconds + 30;
                        break;
                    case "oppo":
                        tempAmplitude = amplitude + 38;
                        tempDuration = milliseconds + 15;
                        break;
                    case "vivo":
                        tempAmplitude = amplitude + 30;
                        tempDuration = milliseconds + 23;
                        break;
                    case "huawei":
                        tempAmplitude = amplitude + 45;
                        tempDuration = milliseconds + 8;
                        break;
                    case "realme":
                        tempAmplitude = amplitude + 34;
                        tempDuration = milliseconds + 19;
                        break;
                    case "oneplus":
                        tempAmplitude = amplitude + 53;
                        tempDuration = milliseconds + 15;
                        break;
                    case "google":
                        tempAmplitude = amplitude + 42;
                        tempDuration = milliseconds + 12;
                        break;
                }

                if (tempAmplitude > 255)
                {
                    tempAmplitude = 255;
                }

                VibrationEffectClassInitialization();
                VibrationEffect = VibrationEffectClass.CallStatic<AndroidJavaObject>("createOneShot", new object[] { tempDuration, tempAmplitude });
        ...
        public static void AndroidVibrate(long[] pattern, int[] amplitudes, int repeat)
        {
            ...
            {
                string manufacturer = new AndroidJavaClass("android.os.Build").GetStatic<string>("MANUFACTURER").ToLower();
                long[] tempDurations = new long[pattern.Length];
                int[] tempAmplitudes = new int[amplitudes.Length];

                // Custom cho từng hãng
                switch (manufacturer)
                {
                    case "samsung":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 38;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 60;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                        }
                        break;
                    case "oppo":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 15;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 38;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    case "vivo":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 23;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 30;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    case "huawei":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 8;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 45;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    case "realme":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 19;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 34;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    case "oneplus":
                        ...
                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 53;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    case "google":
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            if (pattern[i] != 0)
                            {
                                tempDurations[i] = pattern[i] + 12;
                            }
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i] + 42;
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                    ...
                    default:
                        for (int i = 0; i < tempDurations.Length; i++)
                        {
                            tempDurations[i] = pattern[i];
                        }

                        for (int i = 0; i < tempAmplitudes.Length; i++)
                        {
                            tempAmplitudes[i] = amplitudes[i];
                            if (tempAmplitudes[i] > 255)
                            {
                                tempAmplitudes[i] = 255;
                            }
                ...
                VibrationEffectClassInitialization();
                VibrationEffect = VibrationEffectClass.CallStatic<AndroidJavaObject>("createWaveform", new object[] { tempDurations, tempAmplitudes, repeat });
                AndroidVibrator.Call("vibrate", VibrationEffect);
            }
        }
